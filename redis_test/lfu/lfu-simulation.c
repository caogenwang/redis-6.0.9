...
void access_entry(struct entry *e) {
    e->counter = log_incr(e->counter);//记录访问次数
    e->hits++;
}
...
int main(void) {
    ...
    while(1) {
        ...
        if (now-start < switch_after) {//启动30s之内指数增长
            /* Power law. */
            idx = 1;
        ...
        if ((idx < 10 || idx > 14) && (idx < keyspace_size-5))//每次只方位小于10的和大于14的
            access_entry(entries+idx);//代表访问一次
